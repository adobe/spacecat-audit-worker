...
async function runAgenticAndReferralTrafficStep(context) {
  ...
  log.info(`Starting llmo customer analysis audit for site: ${site.getSiteId()}}`);
  ...
  const temporalConditions = lastFourWeeks
    .map(({ year, week }) => getWeekInfo(year, week).temporalCondition);
...
async function runBrandPresenceStep(auditUrl, context, site) {
  const {
    dataAccess, log, scrapeResultPaths, sqs,
  } = context;
  const { Configuration } = dataAccess;
  const configuration = await Configuration.findLatest();
  const domain = site.getBaseURL();
  ...
  try {
    ...
    return {
      status: 'brand presence step completed; no more steps left',
    };
  } catch (error) {
