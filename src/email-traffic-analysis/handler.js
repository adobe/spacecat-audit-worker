...
import { getTotalEmailPageViewsTemplate } from './queries.js';

const { AUDIT_STEP_DESTINATIONS } = Audit;

const IMPORT_TYPE_TRAFFIC_ANALYSIS = 'traffic-analysis';
const AUDIT_TYPE = 'email-traffic-analysis';

const THRESHOLD_LOW = 5000;
const THRESHOLD_HIGH = 30000;
...
function getConfig(env) {
  ...
  if (!bucketName) {
    throw new Error('S3_IMPORTER_BUCKET_NAME must be provided for email-traffic-analysis audit');
  }
...
export function buildMystiqueMessage(site, auditId, baseUrl, auditResult) {
  return {
    type: 'guidance:email-traffic-analysis',
...
export async function prepareEmailTrafficAnalysisRequest(auditUrl, context, site, period) {
  const { log } = context;
  const siteId = site.getSiteId();

  log.debug(`[email-traffic-analysis] Preparing ${period} request parameters for [siteId: ${siteId}] and baseUrl: ${auditUrl}`);
  ...
  log.debug(
    `[email-traffic-analysis] Request parameters: ${JSON.stringify(auditResult)} set for [siteId: ${siteId}] and baseUrl: ${auditUrl}`,
  );
  return {
    auditResult,
...
export async function sendRequestToMystique(auditUrl, auditData, context, site) {
  ...
  log.debug(`[email-traffic-analysis] cache-warming-${period} Starting cache warming for site: ${siteId}`);
  await warmCacheForSite(context, log, env, site, temporalParams);
  log.debug(`[email-traffic-analysis] cache-warming-${period} Completed cache warming for site: ${siteId}`);

  const mystiqueMessage = buildMystiqueMessage(site, id, auditUrl, auditResult);

  log.debug(
    `[email-traffic-analysis] [siteId: ${siteId}] [baseUrl: ${auditUrl}] sending message ${JSON.stringify(mystiqueMessage)} to mystique`,
  );
  await sqs.sendMessage(env.QUEUE_SPACECAT_TO_MYSTIQUE, mystiqueMessage);
  log.debug(`[email-traffic-analysis] [siteId: ${siteId}] [baseUrl: ${auditUrl}] Completed mystique evaluation step`);
}
...
export async function importDataStep(context) {
  ...
  log.info(`[email-traffic-analysis] Starting import data step for siteId: ${siteId}, url: ${finalUrl}`);
  ...
  if (!isImportEnabled(IMPORT_TYPE_TRAFFIC_ANALYSIS, imports)) {
    log.debug(`[email-traffic-analysis] Enabling ${IMPORT_TYPE_TRAFFIC_ANALYSIS} import for site ${siteId}`);
  ...
  log.info(
    `[email-traffic-analysis] [siteId: ${siteId}] Fire-and-forget: ${trendOnlyWeeks.length} trend weeks, chaining decision week 1/4: ${chainedWeek.week}/${chainedWeek.year}`,
  );
  ...
  for (const weekInfo of trendOnlyWeeks) {
    ...
    log.debug(`[email-traffic-analysis] [siteId: ${siteId}] Sending import message for week ${weekInfo.week}/${weekInfo.year}`);
...
export async function importDecisionWeekStep(context) {
  ...
  log.info(
    `[email-traffic-analysis] Chaining decision week ${weekIndex + 1}/4: ${chainedWeek.week}/${chainedWeek.year} for siteId: ${siteId}`,
  );
...
// Queries Athena for total email pageviews, determines decision, generates reports.
...
export async function analyzeAndReportStep(context) {
  ...
  log.info(`[email-traffic-analysis] Starting analyze-and-report step for siteId: ${siteId}`);

  // Decision phase: query Athena for total email pageviews (last 4 weeks)
  ...
  const athenaClient = AWSAthenaClient.fromContext(context, `${config.athenaTemp}/email-traffic-analysis/${siteId}-${Date.now()}`);

  const query = getTotalEmailPageViewsTemplate({
    siteId,
    tableName,
    temporalCondition,
  });

  log.debug(`[email-traffic-analysis] [siteId: ${siteId}] Executing total pageviews query`);

  let result;
  try {
    result = await athenaClient.query(query, config.rumMetricsDatabase, `email-traffic-analysis total pageviews for siteId: ${siteId}`);
  } catch (e) {
    log.error(`[email-traffic-analysis] [siteId: ${siteId}] Athena query failed: ${e.message}`);
    throw e;
  }
  ...
  log.info(`[email-traffic-analysis] [siteId: ${siteId}] totalPageViewSum=${totalPageViewSum}, reportDecision=${reportDecision}`);
  ...
  if (reportDecision === REPORT_DECISION.WEEKLY) {
    const weeklyRequest = await prepareEmailTrafficAnalysisRequest(finalUrl, context, site, 'weekly');
    ...
    log.info(`[email-traffic-analysis] [siteId: ${siteId}] Weekly report generated, auditId=${weeklyAudit.getId()}`);
  }
  ...
  const hasMonthlyReport = existingOpportunities.some((o) => {
    ...
    return o.getType() === 'email-traffic' && data?.month === month && data?.year === monthYear && data?.week == null;
  });

  if (hasMonthlyReport) {
    log.info(`[email-traffic-analysis] [siteId: ${siteId}] Monthly report for ${month}/${monthYear} already exists, skipping`);
  } else {
    const monthlyRequest = await prepareEmailTrafficAnalysisRequest(finalUrl, context, site, 'monthly');
    ...
    log.info(`[email-traffic-analysis] [siteId: ${siteId}] Monthly report generated, auditId=${monthlyAudit.getId()}`);
  }
...
const emailTrafficAnalysis = new AuditBuilder()
...
export default emailTrafficAnalysis;
