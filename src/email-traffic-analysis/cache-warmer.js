...
const EMAIL_DIMENSION_COMBOS = [
  ...
  { dimensions: ['utm_source'], mapper: TrafficDataResponseDto },
  { dimensions: ['utm_source', 'utm_campaign'], mapper: TrafficDataResponseDto },
  { dimensions: ['path'], mapper: TrafficDataWithCWVDto },
  { dimensions: ['path', 'device'], mapper: TrafficDataWithCWVDto },
  { dimensions: ['device'], mapper: TrafficDataResponseDto },
  { dimensions: ['path', 'page_type'], mapper: TrafficDataWithCWVDto },
  { dimensions: ['path', 'page_type', 'device'], mapper: TrafficDataWithCWVDto },
  { dimensions: ['page_type'], mapper: TrafficDataWithCWVDto },
  { dimensions: ['page_type', 'device'], mapper: TrafficDataWithCWVDto },
  { dimensions: ['utm_campaign', 'page_type'], mapper: TrafficDataWithCWVDto },
  { dimensions: ['utm_campaign', 'page_type', 'device'], mapper: TrafficDataWithCWVDto },
];
...
function getConfig(env) {
  const {
    ...
    EMAIL_DATA_THRESHOLD: emailDataThreshold,
  ...
  return {
    ...
    bucketName,
    pageViewThreshold: emailDataThreshold ?? 1000,
    cwvThresholds,
...
export async function warmCacheForSite(context, log, env, site, temporalParams) {
  ...
  log.debug(`Starting email cache warming for site ${siteId}`
    + ` - Year: ${yearInt}, Week: ${weekInt}, Month: ${monthInt}`);
  log.debug(`Using max concurrent requests: ${config.maxConcurrentRequests}`);

  // Check which queries need cache warming
  log.debug(`Checking cache existence for ${EMAIL_DIMENSION_COMBOS.length} queries...`);
  const checkTasks = EMAIL_DIMENSION_COMBOS.map(
  ...
  const warmingTasks = queriesToWarm.map(
    ({ queryConfig }) => () => (async () => {
      try {
  ...
  if (
    queriesToWarm.length > 0
    && successCount === cachedQueries.length
    && noDataErrors.length === failedResults.length
    && failedResults.length > 0
  ) {
    throw new Error(
      `No email traffic data found for site ${siteId}. Please ensure data is imported first before running email traffic analysis.`,
    );
  }
...
export async function warmCacheForQuery(
  ...
  if (!results || results.length === 0) {
    throw new Error(
      `NO_DATA: No email traffic data found for site ${siteId} with dimensions [${dimensions.join(', ')}]. Please ensure data is imported first before running email traffic analysis.`,
    );
  }

  let thresholdConfig = {};
  if (config.cwvThresholds) {
    thresholdConfig = typeof config.cwvThresholds === 'string' ? JSON.parse(config.cwvThresholds) : config.cwvThresholds;
  }
  ...
  if (!response || response.length === 0) {
    throw new Error(
      `NO_DATA: No valid email traffic data found after processing for site ${siteId} with dimensions [${dimensions.join(', ')}]. Please ensure data is imported first before running email traffic analysis.`,
    );
  }
