...
/**
 * Shortlists URLs for the domain-wide RCV suggestion and collects their prompts.
 *
 * Selection algorithm (LLMO-5141):
 *  1. Score each audit result by agenticTraffic * contentGainRatio (combined signal).
 *  2. Take the top DOMAIN_WIDE_URL_SHORTLIST_LIMIT URLs by that score.
 *  3. For each shortlisted URL, look up the corresponding DB suggestion (already
 *     enriched by Mystique) and collect its prompts array.
 *  4. Flatten and cap the total prompt list at DOMAIN_WIDE_MAX_PROMPTS.
 *
 * @param {Array} preRenderSuggestions - Audit result objects (url, contentGainRatio, …)
 * @param {Array} existingDbSuggestions - DB suggestion entities already persisted for this
 *   opportunity (may include Mystique-enriched prompts on their data.prompts field).
 * @param {string} baseUrl - Site base URL
 * @param {Object} log - Logger
 * @returns {Array<string>} shortlistedUrls - The URLs selected for the domain-wide suggestion
 * @returns {Array<string>} prompts - Collected prompts, capped at DOMAIN_WIDE_MAX_PROMPTS
 */
function shortlistUrlsAndCollectPrompts(preRenderSuggestions, existingDbSuggestions, baseUrl, log) {
  // Build a URL → DB suggestion map for O(1) prompt lookup
  const dbSuggestionByUrl = new Map(
    existingDbSuggestions
      .filter((s) => !isDomainWideSuggestionData(s.getData()))
      .map((s) => [s.getData()?.url, s]),
  );

  // Score each audit result and sort descending
  const scored = preRenderSuggestions
    .filter((s) => s.url)
    .map((s) => ({
      url: s.url,
      score: (s.agenticTraffic || 0) * (s.contentGainRatio || 0),
    }))
    .sort((a, b) => b.score - a.score);

  const shortlisted = scored.slice(0, DOMAIN_WIDE_URL_SHORTLIST_LIMIT);

  log.info(
    `${LOG_PREFIX} shortlistUrlsAndCollectPrompts: shortlisted ${shortlisted.length} of `
    + `${preRenderSuggestions.length} URLs for domain-wide suggestion. `
    + `baseUrl=${baseUrl}`,
  );

  // Collect prompts from each shortlisted URL's DB suggestion
  const allPrompts = [];
  for (const { url } of shortlisted) {
    const dbSuggestion = dbSuggestionByUrl.get(url);
    const urlPrompts = dbSuggestion?.getData()?.prompts;
    if (Array.isArray(urlPrompts) && urlPrompts.length > 0) {
      allPrompts.push(...urlPrompts);
    }
  }

  // Cap total prompts
  const prompts = allPrompts.slice(0, DOMAIN_WIDE_MAX_PROMPTS);
  log.info(
    `${LOG_PREFIX} shortlistUrlsAndCollectPrompts: collected ${prompts.length} prompts `
    + `(capped at ${DOMAIN_WIDE_MAX_PROMPTS}) from ${shortlisted.length} shortlisted URLs.`,
  );

  return {
    shortlistedUrls: shortlisted.map((s) => s.url),
    prompts,
  };
}
...
async function prepareDomainWideAggregateSuggestion(
  ...
  const auditedUrlCount = preRenderSuggestions.length;
  ...
  // Shortlist high-value URLs (agentic traffic × content gain) and collect their prompts
  // so the Impact Validation Engine can run a pre/post DRS cycle at domain scope (LLMO-5141).
  const { shortlistedUrls, prompts } = shortlistUrlsAndCollectPrompts(
    preRenderSuggestions,
    existingDbSuggestions,
    baseUrl,
    log,
  ...
  const domainWideSuggestionData = {
    ...
    pathPattern: '/*',
    // Shortlisted URLs and collected prompts for IVE pre/post experiment (LLMO-5141)
    shortlistedUrls,
    prompts,
  };

  log.info(`${LOG_PREFIX} Prepared domain-wide aggregate suggestion for entire domain with allowedRegexPatterns: ${JSON.stringify(allowedRegexPatterns)}. Based on ${auditedUrlCount} audited URL(s). shortlistedUrls=${shortlistedUrls.length}, prompts=${prompts.length}.`);
...
export async function processOpportunityAndSuggestions(
  ...
  // Fetch existing DB suggestions once — used both for domain-wide preservation check
  // and for prompt collection in prepareDomainWideAggregateSuggestion (LLMO-5141).
  const existingDbSuggestions = await opportunity.getSuggestions();

  const existingPreservable = existingDbSuggestions.find(
    (s) => isDomainWideSuggestionData(s.getData()) && shouldPreserveDomainWideSuggestion(s),
  ) || null;

  if (existingPreservable) {
    const status = existingPreservable.getStatus();
    const data = existingPreservable.getData();
    log.info(`${LOG_PREFIX} Found existing domain-wide suggestion to preserve: status=${status}, edgeDeployed=${data?.edgeDeployed}`);
  }
  ...
  await syncSuggestions({
    ...
    scrapedUrlsSet,
    // Pass pre-fetched suggestions to avoid a duplicate DB query (already fetched above)
    existingSuggestions: existingDbSuggestions,
