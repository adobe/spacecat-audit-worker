...
export async function runAuditAndSendUrlsForScrapingStep(context) {
  ...
  let formOpportunities = generateOpptyData(formVitals, undefined, context);
  ...
  formOpportunities = generateOpptyDataForHighPageViewsLowFormNav(formVitals, undefined, context);
  for (const opportunity of formOpportunities) {
    uniqueUrls.add(opportunity.form);
  }
