...
export const handleOutdatedSuggestions = async ({
  ...
  const existingOutdatedSuggestions = existingSuggestions
    ...
    // SITES-44646: when a suggestion's issue is no longer detected, transition it
    // to OUTDATED — including SKIPPED and REJECTED, which are operator decisions
    // on a now non-existent issue (nonsense state; the state IS changing, so
    // updatedAt legitimately bumps). Preserve FIXED (customer already resolved),
    // APPROVED / IN_PROGRESS (in-flight work), OUTDATED (already terminal), and
    // ERROR (needs an explicit retry via defaultMergeStatusFunction).
    .filter((existing) => ![
