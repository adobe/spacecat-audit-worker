  ...
  if (!checks || checks.includes(PREFLIGHT_READABILITY)) {
    ...
    if (step === 'suggest') {
      ...
      if (allReadabilityIssues.length > 0) {
        try {
          log.info(`[preflight-audit] readability: Sending ${allReadabilityIssues.length} readability issues to Mystique for async processing...`);
          ...
          log.info(`[preflight-audit] readability: Successfully sent ${allReadabilityIssues.length} readability issues to Mystique for processing`);

          // Return current results immediately - Mystique will process asynchronously
          // and responses will be handled by guidance-readability-handler.js
          for (const pageResult of auditsResult) {
            const audit = pageResult.audits.find((a) => a.name === PREFLIGHT_READABILITY);
            if (audit && audit.opportunities.length > 0) {
