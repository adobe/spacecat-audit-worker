 ...
 */
function measureInfoGain(original, summary) {
 ...
 */
function categorizeScore(infogainScore) {
 ...
 */
function analyzeContentTraits(content) {
...
export default async function informationGain(context, auditContext) {
  ...
  previewUrls.forEach((url) => {
    ...
    pageResult.audits.push({ name: PREFLIGHT_INFORMATION_GAIN, type: 'seo', opportunities: [] });
  });
  ...
  scrapedObjects.forEach(({ data }) => {
    ...
    // For suggest step, identify weak aspects and provide recommendations
    if (step === 'suggest') {
      ...
      infoGainAudit.opportunities.push({
        ...
        traitScores: analysis.trait_scores,
        weakAspects: weakAspects.map((aspect) => ({
