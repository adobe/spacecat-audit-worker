...
/**
 * @module preflight/image-alt-text
...
import {
  PREFLIGHT_ALT_GUIDANCE_TYPE,
  PREFLIGHT_ALT_OBSERVATION,
  MAX_ALT_TEXT_LENGTH,
} from '../image-alt-text/constants.js';

/**
 * Constant identifier for the image-alt-text preflight audit.
 * Used to register and identify this check in the preflight system.
 * @constant {string}
 */
export const PREFLIGHT_IMAGE_ALT_TEXT = 'alt-text';
...
async function sendToMystiqueForSuggestions(context, site, job, pageUrl, imageIssues) {
  ...
  const imagesNeedingSuggestions = imageIssues;
  ...
  try {
    const mystiqueMessage = {
      type: PREFLIGHT_ALT_GUIDANCE_TYPE,
      siteId,
      auditId: jobId,
      mode: 'preflight',
      deliveryType: site.getDeliveryType(),
      time: new Date().toISOString(),
      observation: PREFLIGHT_ALT_OBSERVATION,
      data: {
        jobId,
        url: pageUrl,
    ...
    log.debug(`[preflight-image-alt-text] Sent ${imagesNeedingSuggestions.length} images to Mystique for suggestions`);

    return { processing: true };
  } catch (error) {
    log.error(`[preflight-image-alt-text] Failed to send to Mystique: ${error.message}`);
    return { processing: false };
  }
}

/**
 * Main handler for the image-alt-text preflight audit check.
 *
...
export default async function imageAltText(context, auditContext) {
  ...
  // Create image-alt-text audit entries for all pages in the audit map
  previewUrls.forEach((url) => {
    const pageResult = audits.get(url);
    pageResult.audits.push({ name: PREFLIGHT_IMAGE_ALT_TEXT, type: 'accessibility', opportunities: [] });
  });
  ...
  previewUrls.forEach((url) => {
    const pageResult = audits.get(url);
    if (pageResult) {
      const imageAltTextAudit = pageResult.audits.find((a) => a.name === PREFLIGHT_IMAGE_ALT_TEXT);
  ...
  for (const { data } of scrapedObjects) {
    ...
    if (!audit) {
      log.warn(`[preflight-image-alt-text] No audit found for URL: ${pageUrl}`);
    ...
    if (imageIssues.length === 0) {
      log.debug(`[preflight-image-alt-text] No image issues found for ${pageUrl}`);
      // eslint-disable-next-line no-continue
      continue;
    }

    log.debug(`[preflight-image-alt-text] Found ${imageIssues.length} image issues for ${pageUrl}`);
  ...
  log.debug(`[preflight-audit] site: ${site.getId()}, job: ${job.getId()}, step: ${step}. Image alt text audit completed in ${imageAltTextElapsed} seconds`);
  ...
  await saveIntermediateResults(context, auditsResult, 'image alt text audit');
