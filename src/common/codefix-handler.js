...
/**
 * Finds suggestions that match the given code change criteria.
...
function findMatchingSuggestions(
  ...
  log.info(`Matched ${matched.length} suggestions for key: ${matchKey}`);
  return matched;
}
...
export async function processCodeFixUpdates(siteId, opportunityId, updates, context) {
  ...
  await Promise.all(updates.map(async (update) => {
    ...
    if (aggregationKey) {
      ...
      // Find matching suggestions and create a FixEntity for this code fix
      const matchedSuggestions = findMatchingSuggestions(
        suggestions,
        url,
        source,
        aggregationKey,
      ...
      if (matchedSuggestions.length > 0) {
        fixEntitiesToCreate.push({
          ...
          suggestions: matchedSuggestions.map((s) => s.getId()),
        });
      }
      return;
    }
    ...
    await Promise.all(types.map(async (ruleId) => {
      ...
      if (matchedSuggestions.length > 0) {
        fixEntitiesToCreate.push({
          ...
          status: 'PENDING',
          changeDetails: {
            description: `Code fix for ${ruleId}`,
            patchContent: reportData.diff,
            ruleId,
          },
          suggestions: matchedSuggestions.map((s) => s.getId()),
  ...
  // Create all FixEntities with their linked suggestions in a single batch
  let totalFixEntities = 0;
  if (fixEntitiesToCreate.length > 0) {
    const { createdItems, errorItems } = await opportunity.addFixEntities(fixEntitiesToCreate);
    totalFixEntities = createdItems.length;

    if (errorItems.length > 0) {
      log.warn(`[CodeFixProcessor] ${errorItems.length} fix entities failed to create: ${errorItems.map((e) => e.error.message).join(', ')}`);
    }
  }

  log.info(`[CodeFixProcessor] Successfully processed all updates. Total fix entities created: ${totalFixEntities}`);
  return totalFixEntities;
}
