...
/**
 * Groups raw categorized error rows by URL, summing hit counts across all LLM
 * user agents and collecting each agent type into a single array.
 *
 * Athena returns one row per (url, user_agent) pair, so the same URL can appear
 * multiple times — once per LLM provider that hit it. Without grouping, passing
 * these rows directly to syncSuggestions would produce duplicate buildKeys and
 * cause data loss (the last row would silently overwrite earlier ones).
 *
 * Example:
 *   /page/foo  ChatGPT   100 hits
 *   /page/foo  Perplexity 50 hits
 *   → one entry: { url: '/page/foo', hitCount: 150, agentTypes: ['ChatGPT', 'Perplexity'] }
 *
 * @param {Array<Object>} errors - Raw categorized error pages from categorizeErrorsByStatusCode.
 *   Each row has: { url, status, agent_type, total_requests, avg_ttfb_ms, country_code,
 *                   product, category }
...
export function groupErrorsByUrl(errors) {
  ...
  // Convert Sets to Arrays for JSON serialisation in the DB
...
/**
 * Parses a period identifier string (as generated by generateReportingPeriods)
 * into a Date representing the Monday of that ISO week.
 *
 * Accepted format: 'wWW-YYYY' (e.g., 'w15-2026').
 * Falls back to epoch (January 1, 1970) for unrecognised formats so that unknown
 * identifiers are always treated as stale by the 4-week cleanup.
 *
 * @param {string} periodIdentifier - Period string in 'wWW-YYYY' format.
 * @returns {Date} Monday of the corresponding ISO week in UTC.
 */
export function parsePeriodIdentifier(periodIdentifier) {
  const match = /^w(\d{2})-(\d{4})$/.exec(periodIdentifier);
  if (!match) {
    return new Date(0); // epoch — treated as stale, will be OUTDATED by cleanup
  }
  ...
  // Monday of week N = Monday of week 1 + (N-1) * 7 days.
  const jan4 = new Date(Date.UTC(year, 0, 4));
  const jan4DayOfWeek = jan4.getUTCDay() || 7; // remap Sunday (0) → 7
