...
export async function refreshGeoBrandPresenceSheetsHandler(message, context) {
  ...
  // Determine if DRS is configured for routing
  const drsClient = DrsClient.createFrom(context);
  const useDrs = drsClient.isConfigured();

  if (useDrs) {
    log.info(`%s: DRS is configured, will route refresh:geo-brand-presence via DRS for siteId: ${siteId}`, AUDIT_NAME);
  } else {
    log.warn(`%s: DRS not configured (DRS_API_URL/DRS_API_KEY absent), falling back to Mystique for siteId: ${siteId}`, AUDIT_NAME);
  }
  ...
  try {
    ...
    const results = await Promise.allSettled(sheets.map(async (sheetName, index) => {
      ...
      if (useDrs) {
        // Route via DRS brand-presence analyze endpoint
        try {
          const drsResult = await drsClient.triggerBrandPresenceAnalyze(siteId, {
            url,
          ...
          log.info(
            `%s: DRS analyze triggered for sheet ${sheetName}, siteId: ${siteId}, jobId: ${drsResult?.job_id}`,
            AUDIT_NAME,
          );
        } catch (drsError) {
          log.error(
        ...
        // Fallback: send via Mystique SQS queue
