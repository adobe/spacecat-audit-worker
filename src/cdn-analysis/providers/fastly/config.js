...
/* c8 ignore start */
export const FASTLY_CONFIG = {
  cdnType: 'fastly',
  databaseName: 'cdn_logs',

  // Raw logs table schema
  rawLogsSchema: {
    timestamp: 'string',
    geo_country: 'string',
    host: 'string',
  ...
  // Filtered logs table schema (same as raw + agentic_type)
  filteredLogsSchema: {
    timestamp: 'string',
    geo_country: 'string',
    host: 'string',
...
export function getFastlyS3Config(context, site = null) {
  ...
  log.info(`Fastly S3 Config: ${customerDomain} (${environment})`, {
...
export function getFastlyCustomerRawLogsLocation(s3Config) {
  return `s3://${s3Config.rawLogsBucket}/raw/`;
}

export function getFastlyRawLogsPartitionConfig(s3Config) {
  const baseLocation = getFastlyCustomerRawLogsLocation(s3Config);
  return {
