...
/* c8 ignore start */
import { getFastlyS3Config } from './config.js';
import { createFastlyFilteredLogsTable, ensureFastlyAthenaTablesExist } from './table-manager.js';
import { FASTLY_AGENTIC_PATTERNS, mapFastlyFieldsForUnload } from './field-mapper.js';
import { executeAthenaQuery } from '../../utils/athena-client.js';
import { getHourlyPartitionFilter } from '../../queries/query-helpers.js';

export const cdnType = 'fastly';

/**
 * Get S3 configuration for Fastly
 */
export function getS3Config(context, site) {
  return getFastlyS3Config(context, site);
}
...
export async function ensureTablesExist(athenaClient, s3Config, log) {
  return ensureFastlyAthenaTablesExist(athenaClient, s3Config, log);
}
...
export async function createFilteredLogsTable(athenaClient, s3Config, log) {
  return createFastlyFilteredLogsTable(athenaClient, s3Config, log);
}
...
export async function filterAndStoreAgenticLogs(
  ...
  try {
    log.info('Filtering and storing Fastly agentic logs to filtered...');
    ...
    const fieldMapping = mapFastlyFieldsForUnload();
    ...
    const unloadQuery = `
      UNLOAD (
        ...
        ${whereClause}
          AND ${FASTLY_AGENTIC_PATTERNS.DETECTION_CLAUSE}
      ) TO '${outputPath}'
      WITH (
        format = 'PARQUET'
      )
    ...
    log.info(`Filtered ${count} Fastly agentic logs and stored to: ${outputPath}`);

    return count;
  } catch (error) {
    log.error('Failed to filter and store Fastly agentic logs:', error);
    throw error;
  }
}
