...
async function run(message, context) {
  ...
  // Handle warmup requests efficiently - no need for full audit processing
  if (type === 'warmup') {
    log.info('Warmup request received - minimal processing');

    // Skip all the heavy initialization for warmup requests
    const warmupHandler = HANDLERS.warmup;
    return warmupHandler(message, context);
  }
