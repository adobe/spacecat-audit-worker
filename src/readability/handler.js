...
async function checkForExistingSuggestions(
  ...
  try {
    ...
    // Check if AsyncJob has a completed result with readability suggestions
    const jobResult = jobEntity?.getResult() || [];
    const hasCompletedResult = jobResult.length > 0 && jobEntity?.getStatus() === 'COMPLETED';

    if (hasCompletedResult) {
      log.info(`[readability-suggest handler] readability: Found completed AsyncJob result with suggestions for jobId: ${jobId}`);

      // Use the completed result from AsyncJob - this handles the case where
      // guidance handler has already processed all Mystique responses
      for (let i = 0; i < auditsResult.length; i += 1) {
        if (jobResult[i] && jobResult[i].audits) {
          const completedReadabilityAudit = jobResult[i].audits.find(
            (a) => a.name === PREFLIGHT_READABILITY,
          );
          const currentReadabilityAudit = auditsResult[i].audits.find(
            (a) => a.name === PREFLIGHT_READABILITY,
          );

          if (completedReadabilityAudit && currentReadabilityAudit) {
            // Replace current audit opportunities with completed ones
            currentReadabilityAudit.opportunities = completedReadabilityAudit.opportunities || [];
            log.info(`[readability-suggest handler] readability: Updated page ${i} with ${currentReadabilityAudit.opportunities.length} completed suggestions`);
          }
        }
      }
      return;
    ...
    const responsesReceived = readabilityMetadata.mystiqueResponsesReceived || 0;
    ...
    log.info(
      ...
      + `for jobId: ${jobId}. Progress: ${responsesReceived}/${responsesExpected} Mystique responses received`,
    );
      ...
      if (audit && audit.opportunities.length > 0) {
        audit.opportunities.forEach((opportunity, index) => {
          ...
          if (matchingSuggestion) {
            ...
            // No suggestion found yet, mark as processing with progress info
            const progressMessage = responsesExpected > 0
              ? `AI suggestions are being generated by Mystique (${responsesReceived}/${responsesExpected} completed). `
                + 'Readability improvements will be available shortly.'
              : 'AI suggestions are being generated by Mystique. '
                + 'Readability improvements will be available shortly.';

            audit.opportunities[index] = {
              ...opportunity,
              suggestionStatus: 'processing',
              suggestionMessage: progressMessage,
              mystiqueRequestSent: readabilityMetadata.lastMystiqueRequest
                || new Date().toISOString(),
            };
          }
        });
      ...
      if (allReadabilityIssues.length > 0) {
        try {
          ...
          if (stillProcessing > 0) {
            ...
            isProcessing = true;
            // Keep opportunities with "processing" status while waiting for Mystique
            // This ensures they appear in the response with appropriate status messages
          } else {
