...
/**
 * @file Test suite for the preflight image-alt-text audit check handler.
 *
 ...
 *
 * @see src/preflight/image-alt-text.js
 */
...
describe('Preflight Image Alt Audit', () => {
  ...
  describe('imageAltText handler', () => {
    it('should create image-alt-text audit entries for all pages', async function () {
      this.timeout(10000); // Allow more time for first dynamic import
      const imageAltText = (await import('../../src/preflight/image-alt-text.js')).default;
      ...
      await imageAltText(context, auditContext);

      // Verify that image-alt-text audit entries were created
    ...
    it('should log warning when no audit found for URL', async () => {
      ...
      expect(log.warn).to.have.been.calledWith(
        '[preflight-image-alt-text] No audit found for URL: https://example.com/unknown-page',
      );
    });
    ...
    it('should log debug message when no image issues found', async () => {
      ...
      expect(log.debug).to.have.been.calledWith(
        '[preflight-image-alt-text] No image issues found for https://example.com/page1',
      );
    });
  });
  ...
  describe('suggest step - Mystique integration', () => {
    it('should send images to Mystique for suggestions in suggest step', async () => {
      ...
      const result = await imageAltText(context, auditContext);
      ...
      expect(message).to.deep.include({
        type: 'guidance:preflight-image-alt-text',
        siteId: 'site-123',
        auditId: 'job-123',
        mode: 'preflight',
    ...
    it('should handle Mystique send error gracefully', async () => {
      ...
      expect(log.error).to.have.been.calledWith(
        '[preflight-image-alt-text] Failed to send to Mystique: SQS error',
      );
