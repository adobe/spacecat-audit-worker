...
// { default: convertToOpportunityStub },
//         '../../../src/image-alt-text/auditEngine.js':
// { default: sandbox.stub().returns(auditEngineStub) },
...
//         sinon.match(/no relevant scraped content found for
// specified pages, cannot proceed with audit/),
//       );
//       expect(convertToOpportunityStub).to.have.been.calledOnce;
//     });
...
//       const expectedMessage = `[${AUDIT_TYPE}]: Found
//  no images without alt text from the scraped content
// in bucket ${bucketName} with path ${s3BucketPath}`;
...
//       expect(context.log.info).to.have.been.calledWith(
//         sinon.match(/Received topPages: 2, includedU
// RLs: 2, totalPages to process after removing duplicates: 3/),
//       );
//     });
...
//       expect(context.log.info).to.have.been.calledWith(
//         sinon.match(/Received topPages: 2, includedURLs:
// 0, totalPages to process after removing duplicates: 2/),
//       );
//     });

//     it('should handle when getIncludedURLs returns null in
// processAltTextAuditStep', async () => {
//       const getIncludedURLsStub = sandbox.stub();
//       getIncludedURLsStub.withArgs('alt-text').returns(null);
//       context.site.getConfig = () => ({
//         getIncludedURLs: getIncludedURLsStub,
...
//       // Should only process top pages
//       expect(context.log.info).to.have.been.calledWith(
//         sinon.match(/Received topPages: 2, includedURLs: 0,
// totalPages to process after removing duplicates: 2/),
//       );
//     });
...
//     it('should not call clearAltTextSuggestions when
// no existing opportunity is found', async () => {
...
//       expect(mockConfiguration.isHandlerEnabledForSite).to
// .have.been.calledWith('alt-text-auto-suggest-mystique', mockSite);
...
//       expect(mockConfiguration.isHandlerEnabledForSite).to.have.been.calledWith
// ('alt-text-auto-suggest-mystique', mockSite);
//       expect(runStub).to.have.been.calledWith(mockMessage, mockContext);
//       expect(result).to.equal(mockAuditResult);
//     });
//   });
