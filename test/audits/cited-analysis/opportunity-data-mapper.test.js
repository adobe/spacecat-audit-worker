...
import { createOpportunityData } from '../../../src/cited-analysis/opportunity-data-mapper.js';

describe('Cited Analysis Opportunity Data Mapper', () => {
  describe('createOpportunityData', () => {
    it('should create opportunity data with defaults when no opportunityData provided', () => {
      ...
      expect(result.type).to.equal('cited-analysis');
      expect(result.status).to.equal('NEW');
      expect(result.runbook).to.equal('');
      expect(result.title).to.equal('LLM discoverability: Improve cited URL presence');
    });

    it('should use values from opportunityData when provided', () => {
      const opportunityData = {
        title: 'Custom Cited Analysis Title',
      ...
      expect(result.tags).to.deep.equal(['cited', 'earned', 'isElmo']);
    ...
    it('should include correct default title and description', () => {
      ...
      expect(result.title).to.equal('LLM discoverability: Improve cited URL presence');
      expect(result.description).to.include('cited URLs');
    ...
    it('should include correct default tags', () => {
      ...
      expect(result.tags).to.include('cited');
    ...
    it('should deduplicate tags when opportunityData contains defaults', () => {
      const opportunityData = {
        tags: ['isElmo', 'cited', 'earned', 'custom'],
      };

      const result = createOpportunityData({ opportunityData });

      expect(result.tags).to.deep.equal(['isElmo', 'cited', 'earned', 'custom']);
    });
