...
describe('Cited Analysis Guidance Handler', () => {
  ...
  describe('Handler with inline analysis data', () => {
    it('should process analysis with suggestions successfully', async () => {
      ...
      expect(funnelingArg.auditType).to.equal(Audit.AUDIT_TYPES.CITED_ANALYSIS);
  ...
  describe('Evergreen opportunity refresh safety', () => {
    ...
    it('should persist a suppressed run as a new opportunity without touching the visible one', async () => {
      ...
      const { mapNewSuggestion } = syncSuggestionsStub.firstCall.args[0];
      expect(mapNewSuggestion({ id: 'test_1', rank: 1, data: {} })).to.not.have.property('status');
...
describe('Cited Analysis URL-index extractors', () => {
