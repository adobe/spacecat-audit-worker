...
import {
  buildTopicCategoryRows, syncTopicCategories,
...
describe('llmo-config-db-sync/sync-junctions', () => {
  ...
  describe('syncTopicCategories', () => {
    ...
    // delete chain uses .eq().in() pattern (grouped by topic_id)
    function makeClient({ fetchData = [], fetchError = null, upsertError = null, deleteError = null } = {}) {
      ...
      return {
        from: sinon.stub().returns(tableBuilder), deleteStub, deleteEqStub, deleteInStub, upsertStub,
      };
    }
    ...
    it('deletes orphaned rows grouped by topic', async () => {
      const { from, deleteEqStub, deleteInStub } = makeClient({
        fetchData: [
          { topic_id: TOPIC_UUID, category_id: 'old-cat-1' },
          { topic_id: TOPIC_UUID, category_id: 'old-cat-2' },
        ],
      });
      const stats = await syncTopicCategories({ from }, config, topicLookup, categoryLookup, log);
      expect(stats.deleted).to.equal(2);
      expect(deleteEqStub).to.have.been.calledWith('topic_id', TOPIC_UUID);
      expect(deleteInStub).to.have.been.calledWith('category_id', ['old-cat-1', 'old-cat-2']);
    });
    ...
    it('throws on delete error', async () => {
      ...
      await expect(syncTopicCategories({ from }, config, topicLookup, categoryLookup, log))
        .to.be.rejectedWith(`Failed to delete topic_categories for topic ${TOPIC_UUID}`);
    });
