...
import { createOpportunityDataForTOC } from '../../../src/toc/opportunity-data-mapper.js';
import { DATA_SOURCES } from '../../../src/common/constants.js';

describe('TOC Opportunity Data Mapper', () => {
  describe('createOpportunityDataForTOC', () => {
    it('should create opportunity data with all required fields', () => {
      const result = createOpportunityDataForTOC();

      expect(result).to.be.an('object');
      expect(result).to.have.property('runbook', '');
      expect(result).to.have.property('origin', 'AUTOMATION');
      expect(result).to.have.property('title', 'Add Table of Content');
      expect(result).to.have.property('description');
      expect(result.description).to.include('table of contents');
    ...
    it('should include correct guidance steps', () => {
      ...
      expect(result.guidance.steps[2]).to.include('head section');
    });
    ...
    it('should include correct tags', () => {
      ...
      expect(result.tags).to.include('Accessibility');
      expect(result.tags).to.include('SEO');
      expect(result.tags).to.include('isElmo');
    });

    it('should have description mentioning TOC implementation', () => {
      const result = createOpportunityDataForTOC();

      expect(result.description).to.include('table of contents');
      expect(result.description).to.include('head section');
    });
  });
});
