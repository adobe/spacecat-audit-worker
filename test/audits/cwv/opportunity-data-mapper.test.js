...
describe('CWV Opportunity Data Mapper', () => {
  describe('createOpportunityData', () => {
    it('should create opportunity data with all required fields', () => {
      ...
      expect(result.runbook).to.include('Experience_Success_Studio_CWV_Runbook');
    ...
    it('should use OPPORTUNITY_TYPES.CWV constant for tags', () => {
    ...
    it('should include kpiDeltas in data object', () => {
      const kpiDeltas = {
        lcpDelta: 0.5,
        clsDelta: 0.1,
        inpDelta: 100,
      ...
      expect(result.data.lcpDelta).to.equal(0.5);
      expect(result.data.clsDelta).to.equal(0.1);
      expect(result.data.inpDelta).to.equal(100);
    ...
    it('should have correct guidance steps', () => {
      ...
      expect(result.guidance.steps).to.have.length(4);
      expect(result.guidance.steps[0]).to.include('Analyze CWV data');
      expect(result.guidance.steps[3]).to.include('Monitor performance');
    });

    it('should handle empty kpiDeltas object', () => {
      const result = createOpportunityData({});
    ...
    it('should handle undefined kpiDeltas', () => {
      const result = createOpportunityData();
