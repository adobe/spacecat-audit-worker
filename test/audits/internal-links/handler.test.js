...
describe('broken-internal-links audit opportunity and suggestions', () => {
  ...
  it('calls reconcileDisappearedSuggestions with correct parameters', async () => {
    const reconcileStub = sandbox.stub().resolves();
    const opportunity = {
      getId: () => 'oppty-1',
    };
    ...
    handler = await esmock('../../../src/internal-links/handler.js', {
      '../../../src/common/opportunity.js': {
        convertToOpportunity: sandbox.stub().resolves(opportunity),
      },
      '../../../src/internal-links/suggestions-generator.js': {
        syncBrokenInternalLinksSuggestions: sandbox.stub().resolves(),
      },
      ...
      '../../../src/utils/data-access.js': {
        reconcileDisappearedSuggestions: reconcileStub,
    ...
    const result = await handler.opportunityAndSuggestionsStep(context);
    expect(result.status).to.equal('complete');
    // Verify reconcileDisappearedSuggestions was called with correct structure
    expect(reconcileStub).to.have.been.calledOnce;
    const callArgs = reconcileStub.getCall(0).args[0];
    expect(callArgs.opportunity).to.equal(opportunity);
    expect(callArgs.currentAuditData).to.deep.equal([{
      urlFrom: 'https://example.com/another',
      urlTo: 'https://example.com/another-to',
      trafficDomain: 1,
    }]);
    expect(typeof callArgs.buildKey).to.equal('function');
    expect(typeof callArgs.getPagePath).to.equal('function');
    expect(typeof callArgs.isIssueFixed).to.equal('function');
    expect(callArgs.auditType).to.equal('broken-internal-links');
  }).timeout(8000);

  // Note: Detailed reconciliation behavior tests have been moved to data-access.test.js
  // since the reconciliation logic is now in the data-access module.

  it('handler completes when reconcileDisappearedSuggestions is called', async () => {
    const reconcileStub = sandbox.stub().resolves();
    ...
    const result = await handler.opportunityAndSuggestionsStep(context);
    expect(result.status).to.equal('complete');
    // Verify reconcileDisappearedSuggestions was called
    expect(reconcileStub).to.have.been.calledOnce;
  }).timeout(8000);

  it('calls publishDeployedFixEntities with isIssueResolvedOnProduction callback', async () => {
    ...
    handler = await esmock('../../../src/internal-links/handler.js', {
      ...
      '../../../src/utils/data-access.js': {
        reconcileDisappearedSuggestions: sandbox.stub().resolves(),
        publishDeployedFixEntities: publishStub,
      },
    });
    ...
    const callArgs = publishStub.getCall(0).args[0];
    expect(callArgs.opportunityId).to.equal('oppty-pub');
    expect(typeof callArgs.isIssueResolvedOnProduction).to.equal('function');
  }).timeout(8000);


  // Note: The checkIfRedirectsToTarget callback tests were removed because esmock cannot properly
  // inject mocks into already-imported modules. The callback uses fetch from the handler's module
  // scope, but reconcileDisappearedSuggestions uses its own imported SuggestionDataAccess.
  // The callback logic is covered by istanbul ignore comments.
