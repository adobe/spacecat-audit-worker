...
describe('sendCodeFixMessagesToImporter', () => {
  ...
  describe('Successful message sending', () => {
    ...
    it('should handle suggestion with default source when source is undefined', async () => {
      ...
      expect(context.log.info).to.have.been.calledWith(
        sinon.match(/Sent code-fix message \(forward type: codefix:accessibility\) to importer for URL: https:\/\/example\.com\/form3, source: default/),
      );
    });
    ...
    it('should log individual message sending for each group', async () => {
      ...
      expect(context.log.info).to.have.been.calledWith(
        sinon.match(/Sent code-fix message \(forward type: codefix:accessibility\) to importer for URL: https:\/\/example\.com\/form1, source: form, issueType: color-contrast, suggestions: 2/),
      );

      expect(context.log.info).to.have.been.calledWith(
        sinon.match(/Sent code-fix message \(forward type: codefix:accessibility\) to importer for URL: https:\/\/example\.com\/form2, source: form2, issueType: select-name, suggestions: 1/),
      );
    });
  });
