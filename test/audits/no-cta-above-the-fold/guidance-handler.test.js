...
describe("No CTA above the fold guidance handler", () => {
  ...
  it("skips opportunity creation when Mystique fails to generate a suggestion", async () => {
    ...
    expect(logStub.info).to.have.been.calledWith(
      '[paid-audit] Skipping no-cta-above-the-fold: suggestion generation failure for site: site-id, url: https://example.com/testpage, audit: audit-id',
    );
  });
...
describe("No CTA above the fold guidance handler - Slack Notifications", () => {
  let sandbox;
  let logStub;
  let context;
  let Audit;
  ...
  beforeEach(async function () {
    ...
    esmockedHandler = mockedModule.default;
  ...
  it("sends Slack notification when slackContext is present in audit result", async () => {
    const audit = {
      getAuditId: () => "audit-id",
      getAuditResult: () => ({
    ...
    expect(mockPostMessageOptional).to.have.been.calledWith(
      context,
      "C123",
      sinon.match(/no-cta-above-the-fold.*audit finished.*example\.com\/testpage/),
      { threadTs: "1234.5678" },
    );
  });

  it("does not send Slack notification when slackContext is absent", async () => {
    const audit = {
      getAuditId: () => "audit-id",
      getAuditResult: () => ({}),
    };
