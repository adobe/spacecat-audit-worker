...
import { warmCacheForSite, warmCacheForQuery } from '../../../src/email-traffic-analysis/cache-warmer.js';

use(sinonChai);
use(chaiAsPromised);

describe('Email-Traffic Analysis Cache Warmer', () => {
  let sandbox;
  let mockS3;
  let mockAthenaQuery;
  let mockAthena;
  ...
  describe('email-traffic warmCacheForSite', () => {
    ...
    it('should return early when all caches exist', async () => {
      ...
      const result = await warmCacheForSite(mockContext, mockLog, mockEnv, mockSite, temporalParams);
    ...
    it('should process only results not already cached', async () => {
      let callCount = 0;
      ...
      expect(warmedResults).to.have.length(12);
      expect(mockAthenaQuery).to.have.been.called;
      expect(mockAthenaQuery.callCount).to.equal(12);
      expect(putObjectCommands).to.have.length(12);
  ...
  describe('edge cases and configuration', () => {
    it('should use defaults for database and threshold when env values missing', async () => {
      ...
      await warmCacheForSite(mockContext, mockLog, envWithOnlyBucket, mockSite, { yearInt: 2025, weekInt: 2, monthInt: 0 });
    ...
    it('should use default max concurrent requests when not provided', async () => {
      ...
      const result = await warmCacheForSite(mockContext, mockLog, envWithoutMax, mockSite, { yearInt: 2025, weekInt: 2, monthInt: 0 });
    ...
    it('should throw error when bucket name is missing', async () => {
      ...
      await expect(
        warmCacheForSite(mockContext, mockLog, envWithoutBucket, mockSite, { yearInt: 2025, weekInt: 2, monthInt: 0 }),
      ).to.be.rejectedWith('S3_IMPORTER_BUCKET_NAME must be provided for caching');

      await expect(
        warmCacheForSite(mockContext, mockLog, { ...mockEnv, S3_IMPORTER_BUCKET_NAME: undefined }, mockSite, {
          yearInt: 2025,
          weekInt: 2,
          monthInt: 0,
        }),
    ...
    it('should handle invalid CWV thresholds and proceed', async () => {
      ...
      const envWithInvalidThresholds = {
        ...mockEnv,
        CWV_THRESHOLDS: '{invalid:json:string}',
      };

      const result = await warmCacheForSite(mockContext, mockLog, envWithInvalidThresholds, mockSite, {
        yearInt: 2025,
        weekInt: 2,
        monthInt: 0,
      });
  ...
  describe('no data scenarios', () => {
    it('should throw error when all queries fail due to no data', async () => {
      ...
      await expect(
        warmCacheForSite(mockContext, mockLog, mockEnv, mockSite, { yearInt: 2025, weekInt: 2, monthInt: 0 }),
      ).to.be.rejectedWith(
        'No email traffic data found for site site-123. Please ensure data is imported first before running email traffic analysis.',
      );

      expect(mockAthenaQuery).to.have.been.called;
    });

    it('should not throw when some queries succeed but others fail due to no data', async () => {
      let callCount = 0;
      ...
      const result = await warmCacheForSite(mockContext, mockLog, mockEnv, mockSite, { yearInt: 2025, weekInt: 2, monthInt: 0 });
  ...
  describe('warmCacheForQuery', () => {
    it('should pass object CWV thresholds to mapper', async () => {
      ...
      expect(mapper.toJSON).to.have.been.calledWith(sinon.match.object, { lcp: 3000, cls: 0.2 }, 'https://example.com');
    });
    ...
    it('should throw NO_DATA error when Athena returns empty results', async () => {
      ...
      await expect(
        ...
        'NO_DATA: No email traffic data found for site site-123 with dimensions [utm_campaign]. Please ensure data is imported first before running email traffic analysis.',
      );
    });
    ...
    it('should throw NO_DATA error when mapper filters out all results', async () => {
      ...
      await expect(
        ...
        'NO_DATA: No valid email traffic data found after processing for site site-123 with dimensions [utm_campaign]. Please ensure data is imported first before running email traffic analysis.',
      );
    });
  });
});
