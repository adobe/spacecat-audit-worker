...
import {
  ...
  prepareEmailTrafficAnalysisRequest,
  sendRequestToMystique,
  buildMystiqueMessage,
  getWeeksForMonth,
} from '../../../src/email-traffic-analysis/handler.js';
...
describe('Email Traffic Analysis Handler', () => {
  let sandbox;
  let clock;
  ...
  describe('buildMystiqueMessage', () => {
    it('should include week number for weekly period', () => {
      ...
      expect(msg.type).to.equal('guidance:email-traffic-analysis');
  ...
  describe('prepareEmailTrafficAnalysisRequest', () => {
    it('should prepare weekly analysis request correctly', async () => {
      const site = createSite(sandbox);
      const context = { log: { debug: sandbox.spy() } };

      const result = await prepareEmailTrafficAnalysisRequest(auditUrl, context, site, 'weekly');
    ...
    it('should prepare monthly analysis request correctly', async () => {
      ...
      const result = await prepareEmailTrafficAnalysisRequest(auditUrl, context, site, 'monthly');
  ...
  describe('sendRequestToMystique', () => {
    ...
    it('should handle SQS errors gracefully', async () => {
      ...
      await expect(sendRequestToMystique(auditUrl, auditData, context, site)).to.be.rejectedWith('SQS Error');
    });
  });
  ...
  describe('importDataStep', () => {
    ...
    it('should throw error when site config is null', async () => {
      ...
      await expect(importDataStep(context)).to.be.rejectedWith(/site config is null/);
    });
    ...
    it('should return correct structure for import worker chaining', async () => {
      ...
      expect(result).to.have.all.keys('auditResult', 'fullAuditRef', 'type', 'siteId', 'allowCache', 'auditContext');
  ...
  describe('analyzeAndReportStep', () => {
    ...
    describe('decision logic', () => {
      it('should return NOT_ENOUGH_DATA when totalPageViewSum < 5K', async () => {
        const ctx = createAnalyzeContext([{ total_pageview_sum: '3000' }]);

        const result = await analyzeAndReportStep(ctx);

        expect(result.auditResult.totalPageViewSum).to.equal(3000);
      ...
      it('should return MONTHLY when totalPageViewSum is exactly 5K', async () => {
        const ctx = createAnalyzeContext([{ total_pageview_sum: '5000' }]);
      ...
      it('should return WEEKLY when totalPageViewSum is exactly 30K', async () => {
    ...
    describe('MONTHLY decision', () => {
      it('should create 1 child audit (monthly) and send 1 Mystique message', async () => {
        ...
        const ctx = createAnalyzeContext([{ total_pageview_sum: '10000' }], {
          dataAccess: {
            Audit: { create: createStub },
        ...
        expect(createArgs.auditType).to.equal('email-traffic-analysis');
    ...
    describe('child audit records', () => {
      it('should have correct fields in child audit records', async () => {
        ...
        expect(weeklyArgs.auditType).to.equal('email-traffic-analysis');
        ...
        expect(monthlyArgs.auditType).to.equal('email-traffic-analysis');
    ...
    describe('error handling', () => {
      it('should throw when S3_IMPORTER_BUCKET_NAME is missing', async () => {
        ...
        await expect(analyzeAndReportStep(ctx)).to.be.rejectedWith('S3_IMPORTER_BUCKET_NAME must be provided');
      });
      ...
      it('should log contextual error and re-throw when Athena query fails', async () => {
        ...
        await expect(analyzeAndReportStep(ctx)).to.be.rejectedWith('Athena connection failed');

        expect(ctx.log.error).to.have.been.calledWith(sinon.match(`[siteId: ${siteId}] Athena query failed`));
      });
