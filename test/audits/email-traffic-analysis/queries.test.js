...
import { getTotalEmailPageViewsTemplate } from '../../../src/email-traffic-analysis/queries.js';

describe('Email Traffic Analysis Queries', () => {
  const defaultParams = {
    siteId: 'test-site',
  ...
  describe('getTotalEmailPageViewsTemplate', () => {
    it('should generate valid SQL with SUM of pageviews', () => {
      const query = getTotalEmailPageViewsTemplate(defaultParams);
    ...
    it('should use provided siteId', () => {
      ...
      const query = getTotalEmailPageViewsTemplate(params);
    ...
    it('should filter by owned traffic type', () => {
      const query = getTotalEmailPageViewsTemplate(defaultParams);

      expect(query).to.include("trf_type = 'owned'");
    });

    it('should filter by email utm_medium', () => {
      const query = getTotalEmailPageViewsTemplate(defaultParams);

      expect(query).to.include("utm_medium = 'email'");
    });
  });
});
