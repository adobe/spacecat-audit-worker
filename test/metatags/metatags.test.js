...
describe('Meta Tags', () => {
  describe('SeoChecks', () => {
    ...
    describe('buildElementsPayload', () => {
      beforeEach(() => {
        // Setup page selectors
        seoChecks.pageSelectors = {
          '/page1': {
            title: ['title'],
            description: ['meta.description:nth-of-type(1)'],
            h1: ['h1:nth-of-type(1)', 'h1:nth-of-type(2)'],
          },
          '/page2': {
            title: [],
      ...
      it('should return empty object when selectors is null', () => {
        // Override the selectors to be null for this path/tag
        seoChecks.pageSelectors['/page-no-selectors'] = { title: null };
        const result = seoChecks.buildElementsPayload('/page-no-selectors', 'title');
        expect(result).to.deep.equal({});
      });

      it('should return empty object when selectors is undefined', () => {
        const result = seoChecks.buildElementsPayload('/nonexistent', 'title');
        expect(result).to.deep.equal({});
      });

      it('should return elements when selectors exist', () => {
        const result = seoChecks.buildElementsPayload('/page1', 'description');
        expect(result).to.have.property('elements');
        expect(result.elements).to.be.an('array');
        expect(result.elements).to.have.lengthOf(1);
        expect(result.elements[0]).to.deep.equal({ selector: 'meta.description:nth-of-type(1)' });
      });

      it('should return first selector only when firstOnly option is true', () => {
        const result = seoChecks.buildElementsPayload('/page1', 'h1', { firstOnly: true });
        expect(result).to.have.property('elements');
        expect(result.elements).to.have.lengthOf(1);
        expect(result.elements[0]).to.deep.equal({ selector: 'h1:nth-of-type(1)' });
      });

      it('should return all selectors when firstOnly option is false', () => {
        const result = seoChecks.buildElementsPayload('/page1', 'h1', { firstOnly: false });
        expect(result).to.have.property('elements');
        expect(result.elements).to.have.lengthOf(2);
        expect(result.elements[0]).to.deep.equal({ selector: 'h1:nth-of-type(1)' });
        expect(result.elements[1]).to.deep.equal({ selector: 'h1:nth-of-type(2)' });
      });

      it('should return empty object when selectors array is empty', () => {
        const result = seoChecks.buildElementsPayload('/page2', 'title', { firstOnly: true });
        expect(result).to.deep.equal({});
      });

      it('should return empty object when firstOnly is true but selectors array is empty', () => {
        const result = seoChecks.buildElementsPayload('/page2', 'title', { firstOnly: true });
        expect(result).to.deep.equal({});
      });

      it('should handle single selector', () => {
        const result = seoChecks.buildElementsPayload('/page3', 'h1');
        expect(result).to.have.property('elements');
        expect(result.elements).to.have.lengthOf(1);
        expect(result.elements[0]).to.deep.equal({ selector: 'h1' });
      });
    });
  });
