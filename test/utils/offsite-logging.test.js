...
describe('offsite-logging helper', () => {
  ...
  describe('withAuditPersistLog', () => {
    it('is a post-processor that logs audit_persistence_run_persisted success from the persisted audit data', async () => {
      ...
      expect(log.info).to.have.been.calledOnceWithExactly(
        '[offsite:cited] Audit persisted domain=offsite audit=cited event=audit_persistence_run_persisted '
    ...
    it('falls back to context.audit for the id when auditData.id is absent', async () => {
      ...
      expect(log.info).to.have.been.calledOnceWithExactly(
        '[offsite:brand-presence] Audit persisted domain=offsite audit=brand-presence event=audit_persistence_run_persisted '
