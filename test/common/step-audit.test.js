...
describe('Step-based Audit Tests', () => {
  ...
  describe('Step Execution', () => {
    ...
    it('routes to next step via SQS when SCRAPE_CLIENT step has no URLs', async () => {
      nock('https://space.cat')
        .get('/')
        .reply(200, 'Success');

      const mockScrapeClient = {
        createScrapeJob: sandbox.stub().resolves({ id: 'scrape-job-bypass' }),
      };
      sandbox.stub(ScrapeClient, 'createFrom').returns(mockScrapeClient);

      const bypassAudit = new AuditBuilder()
        .addStep('scrape-step', async () => ({
          urls: [],
          siteId: '42322ae6-b8b1-4a61-9c88-25205fa65b07',
        }), AUDIT_STEP_DESTINATIONS.SCRAPE_CLIENT)
      ...
      const result = await bypassAudit.run(scrapeMessage, context);

      expect(result.status).to.equal(200);

      // ScrapeClient must NOT be invoked
      expect(mockScrapeClient.createScrapeJob).not.to.have.been.called;

      // A continuation SQS message must be sent directly to AUDIT_JOBS_QUEUE_URL
      expect(context.sqs.sendMessage).to.have.been.calledOnce;
      const [queueUrl, payload] = context.sqs.sendMessage.firstCall.args;
      expect(queueUrl).to.equal('https://space.cat/audit-jobs');
      expect(payload.type).to.equal('content-audit');
      expect(payload.siteId).to.equal('42322ae6-b8b1-4a61-9c88-25205fa65b07');
