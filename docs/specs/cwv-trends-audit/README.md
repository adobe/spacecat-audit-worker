...
| **Custom `mergeDataFunction`** | The default shallow merge in `syncSuggestions` wouldn't update `suggestionValue` on re-runs since the raw device result doesn't contain that key. The custom function JSON-stringifies the new result into `suggestionValue` (required for UI compatibility - frontend expects a JSON string) |
| **Null values default to `0`** | The UI calls `.toFixed()` on numeric fields without null guards. All numeric fields (`lcp`, `cls`, `inp`, `*Change`, etc.) default to `0` |
| **URLs with no CWV data filtered** | URLs where all three CWV metrics (lcp, cls, inp) are null are filtered out after the pageviews check. Absence of measurement is not the same as good performance |
| **URL normalization** | URLs are normalized via `new URL(url).href` after validation to percent-encode special characters and prevent XSS vulnerabilities |
| **Status validation** | The `newSuggestionStatus` parameter in `syncSuggestions` is validated against the allowed statuses enum, preventing arbitrary strings from being passed |
